...
export class ClaudeProvider extends AIProvider {
	...
	async chat(messages: AIMessage[]): Promise<string> {
		...
		try {
			...
			console.log(
				`✅ Claude request successful. Remaining: ${this.rateLimiter.getRemainingRequests()} requests, ${this.rateLimiter.getRemainingTokens()} tokens`,
			);
			...
			if (error instanceof Error) {
				...
				if (error.message.includes('rate_limit')) {
					throw new Error(
						`Claude rate limit exceeded. Please wait before making another request.`,
					);
				}
				if (error.message.includes('insufficient_quota')) {
					throw new Error(
						`Claude quota exceeded. Please check your billing settings.`,
					);
				}
				if (error.message.includes('authentication_error')) {
					throw new Error(
						`Invalid Claude API key. Please check your ANTHROPIC_API_KEY environment variable.`,
					);
				}
