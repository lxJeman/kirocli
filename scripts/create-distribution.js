...
function createPortablePackage(platform) {
	...
	const minimalPackageJson = {
		...
		description:
			originalPackageJson.description || 'AI Developer Terminal Copilot',
		type: 'module',
		bin: 'dist/cli.js',
	...
	try {
		...
		log(
			'⚠️ Failed to install dependencies, copying from main project',
			'yellow',
		);
...
function createArchive(packageDir, platform) {
	...
	try {
		execSync(
			`tar -czf "${archiveName}" -C "${dirname(packageDir)}" "${
				platform === 'windows'
					? 'kirocli-windows-portable'
					: `kirocli-${platform}-portable`
			}"`,
			{
				cwd: rootDir,
				stdio: 'inherit',
		...
		log(
			`⚠️ Failed to create archive, package available as directory`,
			'yellow',
		);
		return packageDir;
	}
